...
export function Dashboard() {
  ...
  useEffect(() => {
    fetchSeasonalityL2Categories()
      .then((r) => setL2Options(r.categories ?? []))
      .catch(() => setL2Options([]));
  }, []);
    ...
    <div className="space-y-10 pb-12">
          ...
          {effectiveL2Options.length ? (
            <label className="flex flex-col gap-1 text-[11px] font-medium uppercase tracking-wide text-slate-500">
              ...
              <select
                ...
                className="min-w-[200px] rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm font-normal text-slate-200 outline-none focus:border-slate-500"
              >
      ...
      <section className="space-y-6">
        ...
        <div>
          ...
          <ChartCard title="Share of revenue (top categories)" subtitle="Each slice is a category&apos;s share of revenue.">
            <div className="h-80">
              {pie.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
